...
void createDatabase(const std::string& dbPath) {
    ...
    const char* createTableSQL = "CREATE TABLE IF NOT EXISTS Personen(" \
    ...
    const char* insertSQL = "INSERT INTO Personen (Name, Alter) VALUES " \
...
void queryDatabase(const std::string& dbPath) {
    ...
    const char* querySQL = "SELECT ID, Name, Alter FROM Personen;";
    sqlite3_stmt* stmt;
